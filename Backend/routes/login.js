...
router.post("/admin", async (req, res) => {
  ...
  if (await bcrypt.compare(req.body.password,logedAdmin.password)&&logedAdmin.isAdmin){
    return res.status(200).send("Welcome Admin :)");
  }else{
    return res.status(500).send("Make sure that your password true Or you are not Admin :( ")
  }
});
